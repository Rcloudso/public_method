...
# @Time    : 2022/3/23
# @Author  : 阿宋
# @File    : exchange_code_batch_list.py
...
class ExchangeBatchList(KeyWeb):
    exchange_batch_url = base_url + "exchange/ExchangeBatchList"
    ...
    # 新增确定按钮,由于先尝试删除再新增，所以这里用last最后一个尝试
    ...
    def search_ac_cp(self, ac_name):
        self.open_url(self.exchange_batch_url)
        log.info("已打开兑换码批次管理页面，url为: %s" % self.exchange_batch_url)
    ...
    def edit_batch_goods(self, ac_name):
    ...
    def delete_batch_exchange(self, ac_name):
        self.search_ac_cp(ac_name)
        self.del_batch()
        return self.assert_result()
